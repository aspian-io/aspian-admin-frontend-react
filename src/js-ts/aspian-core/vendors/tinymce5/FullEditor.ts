...
const FullTextEditorInit = (selectorId: string, height: number = 250, dir: DirectionActionTypeEnum = DirectionActionTypeEnum.LTR, lang: LanguageActionTypeEnum = LanguageActionTypeEnum.en, initialContent: string = '') => {
  const editorDir = dir === DirectionActionTypeEnum.RTL ? "rtl" : "ltr";
  const editorLang = lang === LanguageActionTypeEnum.fa ? "fa" : null;
  tinymce.init({
    selector: `#${selectorId}`,
    ...
    setup: (editor: any) => {
      editor.on("keyup", (e: any) => {
        ...
        const editorTextareaTag = document.querySelector("#addPostEditor");
    ...
    tinymce.get( selectorId ).setContent( initialContent );
  });
};
