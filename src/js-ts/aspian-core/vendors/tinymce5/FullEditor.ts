...
const mceCallbackInvoker = (mceCallback: any) => {
    document.querySelectorAll(".insertFileToMce").forEach((el) => {
        el.addEventListener("click", () => {
            setTimeout(() => {
                mceCallback(store.getState().attachment.lastChosenFileKey, {title: store.getState().attachment.lastChosenFileKey});
            }, 500);
        })
    })
}
...
const FullTextEditorInit = (selectorId: string,
                            ...
                            setPostContent: (postEditorContent: string) => ISetPostContentAction
) => {
    // Redux states
    ...
    tinymce.init({
        ...
        setup: (editor: any) => {
            editor.on("keyup", (e: any) => {
                ...
                //requiredInputsOnChange(content);
        ...
        file_picker_callback: function (callback: any, value: any, meta: any) {
            ...
            if (meta.filetype === 'file') {
                showFileBrowserModal(FileBrowserModalTypeEnum.MISCELLANEOUS_FILE_BROWSER);
                if (store.getState().attachment.miscellaneousFileBrowserDataSource.length === 0) {
                    fileBrowser(AttachmentTypeEnum.Other).then(() => {mceCallbackInvoker(callback)});
                } else {
            ...
            if (meta.filetype === 'image') {
                showFileBrowserModal(FileBrowserModalTypeEnum.PHOTO_FILE_BROWSER);
                if (store.getState().attachment.photoFileBrowserDataSource.length === 0) {
                    fileBrowser(AttachmentTypeEnum.Photo).then(() => {mceCallbackInvoker(callback)});
                } else {
                    mceCallbackInvoker(callback);
                }
            }
            ...
            if (meta.filetype === 'media') {
                showFileBrowserModal(FileBrowserModalTypeEnum.VIDEO_FILE_BROWSER);
                if (store.getState().attachment.videoFileBrowserDataSource.length === 0) {
                    fileBrowser(AttachmentTypeEnum.Video).then(() => {mceCallbackInvoker(callback)});
                } else {
                    mceCallbackInvoker(callback);
                }
            }
        ...
        tinymce.get(selectorId).setContent(state.post.postEditorContent);
    });
};
